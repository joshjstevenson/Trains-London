...
/**
* NaPTAN identifiers are a systematic way of identifying of all UK points of access to public 
* transport or "Stop points"). Every UK rail station, bus and coach terminus, airport, ferry terminal, 
* individual bus stop, tram stop, and taxi rank is allocated a unique NaPTAN Identifier.
*/

// copy and pasted from 'stations-London.csv' file.
// lazy and wrong way to do it?? I'm not sure...
const csvFile = `
name,line,naptanId
 ...
 * an API request; naptanID and line. If no match is found, throw an error.
 */

function processInput() {
    // take the user input from HTML form expecting a station name.
    // format user input for consistency.
    // save as String variable 'stationName'.
    const userInput = document.getElementById('station').value;
    stationName = userInput.replace(/\b\w/g, char => char.toUpperCase());

    stationInformation(stationName);
}

function stationInformation(stationName) {
    // search stationObjects array with stationName to find the 
    // corresponding naotanId and line information.
    const result = stationObjects.find(obj => obj.name === stationName);
    if (result) {
        const stationId = result.naptanID;
        const stationLine = result.line;
    } else {
        console.log('Error: No matching station found');
    }

    stationSearch(stationLine, stationId);
}

// my app_keys:
// fe96f0290c804e9b93203a34f4e3fb2c
// 
function stationSearch(stationLine, stationId) {
    // using the stationLine and stationId variables, append a TFL API call.
    // return relevant response data to then be formatted appropriately
    // on the HTML webpage.
    const response = fetch(`https://api.tfl.gov.uk/Line/${stationLine}/Arrivals/${stationId}?direction=all&app_id={{app_id}}&app_key=fe96f0290c804e9b93203a34f4e3fb2c`, {
        method: 'GET',
        // Request headers
        headers: {
...
// pull the relevant data we want from the API response and display where necessary.